...
你可能会好奇这么一个哈夫曼树是怎么构造的，其实它是按照一个**贪心思想**和规则构造，而构造出来的这个树的权值最小。
...
例如上面 **2，3，6，8，9**权值节点构成的哈夫曼树的WPL计算为(设根为第0层)：`2*3+3*3+6*2+8*2+9*2=(2+3)*3+(6+8+9)*2=61`。
...
![](./assets/img-ypy-r.png)

![](./assets/img-ypy-t.png)
...
public class HuffmanTree {
    ...
    public void createTree() {
        Queue<node> q1 = new PriorityQueue<>(Comparator.comparingInt(o -> o.value));
        q1.addAll(nodes);
        while (!q1.isEmpty()) {
            node n1 = q1.poll();
            node n2 = q1.poll();
            node parent = null;
            if (n2 != null) {
                parent = new node(n1, n2, n1.value + n2.value);
            }
            if (q1.isEmpty()) {
                root = parent;
                return;
    ...
    public static void main(String[] args) {
        ...
        tree.nodes = list;
        tree.createTree();
        System.out.println(tree.get_weight());
    }
}
```
...
哈夫曼编码的目的是为了减少存储体积，以一个连续的字符串为例，抛开编程语言中实际存储，就拿`aaaaaaaaaabbbbbcccdde`这个字符串来说，在计算机中如果每个字符都是定长存储(假设长为4的二进制存储)，计算机只知道0和1的二进制，假设
...
![image-20210616134909721](./assets/image-20210616134909721.png)
