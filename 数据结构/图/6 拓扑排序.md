...
很多人不知道AOE网干啥用的，拓扑排序是解决一个工程能否顺序进行的问题，但有时还需解决工程完成需要的最短时间。而AOE经常使用在求**关键路径**中(这里就先不进行详细介绍内容和算法了)。

![](./assets/img-wtup-w.png)
    ...
    public static void main(String[] args) {
        ...
        while (!s1.isEmpty()) {
            ...
            for (Integer integer : next) {
                a[integer]--;//入度减一
                if (a[integer] == 0)//如果入度为0
                {
                    s1.add(nodes[integer]);
                }
            }
        }
    }
...
class Solution {
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for (int[] prerequisite : prerequisites) {
            int pre_id = prerequisite[1];
            int course_id = prerequisite[0];
            indegree[course_id]++;//入度加一
            next[pre_id].add(course_id);//next指向
        }
