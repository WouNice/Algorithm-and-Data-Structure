...
布隆过滤器有着广泛的应用，对于大量数据的“存不存在”的问题在空间上有明显优势，但是在判断存不存在是有一定的错误率(false positive)，也就是说，有可能把不属于这个集合的元素误认为属于这个集合(False Positive)，但不会把属于这个集合的元素误认为不属于这个集合(False Negative)。布隆过滤器由来
...
$\left[1-(1+\frac{1}{(-m)})^{(-m)\frac{kn}{(-m)}}\right]^k\approx(1-e^{-\frac{kn}{m}})^k$
...
$m\geq\log_{2}\frac{\binom{u}{n}}{\binom{n+\epsilon(u-n)}{n}}\approx\log_{2}\frac{\binom{u}{n}}{\binom{\epsilon u}{n}}\geq\log_{2}\epsilon^{-n}=n\log_{2}(1/\epsilon).$
...
$m\geq n\frac{\log_{2}(1/\epsilon)}{\ln2}=n\log_{2}\mathbf{e}\cdot\log_{2}(1/\epsilon).$
...
$\mathbb{P}(c(i)\geq j)\leq\binom{nk}{j}\frac{1}{m^j}\leq\left(\frac{\mathbf{e}nk}{jm}\right)^j.$
...
$\mathbb{P}(\max_ic(i)\geq j)\leq m\left(\frac{\mathrm{e}\ln2}{j}\right)^j.$

如果每个Counter分配4位，那么当Counter的值达到16时就会溢出。这个概率为：

$\mathbb{P}(\max_ic(i)\geq16)\leq1.37\times10^{-15}\times m.$
